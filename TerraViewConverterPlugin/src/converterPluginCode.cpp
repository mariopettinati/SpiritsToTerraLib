#include <converterPluginCode.h>

#include <converterPlugin.h>

static converterPlugin* converterPlugin_plg = 0;
...
SPL_PLUGIN_API bool SPL_INIT_NAME_CODE( slcPluginArgs* a_pPluginArgs )
{
	...
	if( a_pPluginArgs->GetArg( 0, arg_ptrs ) == 0 )
	{
		...
		if( params->getVersion() == PLUGINPARAMETERS_VERSION )
		{
			...
			converterPlugin_plg = new converterPlugin(params);

			converterPlugin_plg->init();
		}
	}
...
{
	delete converterPlugin_plg;
	converterPlugin_plg = 0;
